...
(NULL, '事件中心', 'events',     'MODULE', '查看视觉识别事件和语音告警播报'),
...
-- 3.3 事件中心操作级权限（1 个）
INSERT INTO `permission` (parent_id, name, permission_code, type, description)
SELECT p.id, '事件查看', 'events:read', 'ACTION', '查看视觉/语音事件列表' FROM `permission` p WHERE p.permission_code = 'events';
...
(NULL, '事件中心', 'events',      'eye',      '/events',     'events/index',     6, 1),
(NULL, '策略配置', 'policy',      'strategy', '/strategy',   'strategy/index',   7, 1),
(NULL, '智能助手', 'assistant',   'robot',    '/assistant',  'assistant/index',  8, 1),
(NULL, '系统日志', 'audit',       'history',  '/logs',       'logs/index',       9, 1),
(NULL, '用户管理', 'user',        'user',     '/users',      'system/user/index',10, 1),
(NULL, '系统管理', NULL,          'setting',  '/system',     NULL,              11, 1);
...
-- 5.2 MUNICIPAL（市政人员）：大屏/设备查看控制/遥测/能耗/告警处理/事件查看/策略查看/助手/日志
INSERT INTO `role_permission` (role_id, permission_id)
...
-- 5.3 MAINTENANCE（路灯管理员）：大屏/设备全部/遥测/能耗/告警全部/事件查看/策略查看/助手
INSERT INTO `role_permission` (role_id, permission_id)
SELECT r.id, p.id FROM `role` r, `permission` p
...
-- 5.4 EMERGENCY（安全/应急人员）：大屏/告警查看/事件查看/助手
INSERT INTO `role_permission` (role_id, permission_id)
SELECT r.id, p.id FROM `role` r, `permission` p
